...
import { resolveProvider, detectBrainPathForProvider } from './providers.js';
 ...
 * @param {string|null} [lang=null] - Optional active language override.
 * @returns {{ handoffPath: string, continueHerePath: string }}
 */
export function generateHandoff(planningPath, brainPath = null, lang = null) {
  if (!planningPath) {
  ...
  const lines = [
    t('handoff_md_title', {}, resolvedLang),
    t('handoff_md_auto_gen', { date: handoff.generatedAt }, resolvedLang),
  ];

  if (currentState) {
    lines.push(t('handoff_md_current_state', {}, resolvedLang), '', currentState, '');
  }

  if (currentPlan) {
    lines.push(t('handoff_md_current_plan', {}, resolvedLang), '', currentPlan.slice(0, 2000), '');
  }

  if (brainArtifacts.length > 0) {
    lines.push(t('handoff_md_brain_artifacts', {}, resolvedLang), '');
    for (const a of brainArtifacts) {
      const summaryText = a.summary || t('handoff_md_no_summary', {}, resolvedLang);
      lines.push(t('handoff_md_brain_art_item', { name: a.name, type: a.type, summary: summaryText }, resolvedLang));
    }
    lines.push('');
  }

  if (recentLog.length > 0) {
    lines.push(t('handoff_md_recent_activity', {}, resolvedLang), '');
  ...
  if (brainPath) {
    lines.push(t('handoff_md_brain_path', {}, resolvedLang), '', `\`${brainPath}\``, '');
  }
...
/**
 * Automatically detect the active AI CLI brain folder based on
 * the most recently updated conversation directory.
 *
 * Supports multiple providers (Antigravity, Claude Code, Codex CLI).
 * When no provider is specified, auto-detects based on available data.
 *
 * @param {string|null} [providerOverride=null] - Provider name override ("antigravity", "claude", "codex", "auto", or null)
 * @returns {string|null} The resolved brain path, or null if not found.
 */
export function detectBrainPath(providerOverride = null) {
  try {
    const { key } = resolveProvider(providerOverride);
    return detectBrainPathForProvider(key);
  } catch {
    // Fallback: if providers.js fails or doesn't exist, use original hardcoded path
    ...
    try {
      const items = fs.readdirSync(brainBaseDir);
      let latestDir = null;
      let latestTime = 0;

      for (const item of items) {
        const itemPath = path.join(brainBaseDir, item);
        let stat;
        try {
