...
import { createSnapshot, runGit } from '../helpers.js';
import { resolveProvider, isExecutableAvailable } from '../providers.js';

export async function run({ targetDir, _activeLang, cleanArgs }) {
    ...
    try {
        ...
        if (!isExecutableAvailable(swarmProvider.executable)) {
                        ...
                        try {
                            task.files.forEach(f => {
                                if (fs.existsSync(path.join(targetDir, f))) {
                                    runGit(['add', f], { cwd: targetDir });
                                }
                            });
                            runGit(['commit', '-m', `yapu: ${task.id} - ${task.name}`], { cwd: targetDir, stdio: 'ignore' });
